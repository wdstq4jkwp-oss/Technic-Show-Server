...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def init_ui(self):
        ...
        self.ui.action_restart_clever.triggered.connect(
            b_partial(self.send_to_selected, "service_restart", command_kwargs={"name": "technic"}))
    ...
    def client_connection_changed(self, client: Client):
        ...
        if row_data is None:
            logging.debug("Client {} is not in the table yet".format(client))
            return
        ...
        if client.connected:
            self.model.update_data(row_num, 0, True, table.ModelStateRole)
        else:
            self.model.update_data(row_num, 0, False, table.ModelStateRole)
            for col, name in enumerate(self.model.columns):
                if name == "copter_id":
                    continue
                self.model.update_data(row_num, col, "NO_FCU" if name == "fcu_status" else None,
                                       table.ModelDataRole)
                self.model.update_data(row_num, col, False, table.ModelStateRole)

        logging.debug("Client status updated")   # ← 8 пробелов, ВНУТРИ метода

    @pyqtSlot()
    def selfcheck_selected(self):                # ← 4 пробела, метод класса
        for copter in self.model.user_selected():
    ...
    def _send_files(self, files, copters=None, client_path="", client_filename="", match_id=False, callback=None, technic_dir=False):
        if copters is None:
        ...
        for num, file in enumerate(files):
            ...
            for copter in to_send:
                if technic_dir:
                    if copter.client.technic_dir != 'error':
                        path_to_send = PurePosixPath(copter.client.technic_dir) / client_path
                    else:
                        logging.error("Can't send files to technic ROS package on {}".format(copter.copter_id))
                else:
    ...
    def send_files(self, prompt, ext_filter, copters=None, client_path="", client_filename="", match_id=False,
                   onefile=False, callback=None, technic_dir=False):
        if onefile:
        ...
        self._send_files(files, copters, client_path, client_filename, match_id, callback, technic_dir)

    def send_directory_files(self, prompt, extensions=(), copters=None, client_path="", client_filename="",
                             match_id=False, callback=None, technic_dir=False):
    ...
    def send_calibrations(self):
        self.send_directory_files("Select directory with calibrations", ('.yaml', ), match_id=True,
                                  client_path="camera_info", client_filename="calibration.yaml", technic_dir=True)

    @pyqtSlot()
    def send_aruco(self):
        def callback(copter):
            copter.client.send_message("service_restart", kwargs={"name": "technic"})
    ...
    def restart_clever_show(self):
        for copter in self.model.user_selected():
            copter.client.send_message("service_restart", kwargs={"name": "technic-show"})
...
if __name__ == "__main__":
    ...
    splash.showMessage("Loading technic-show server"+"\n\n\n\n\n", int(Qt.AlignBottom | Qt.AlignCenter), Qt.white)
    app.processEvents()
    splash.show()
    # time.sleep(3)

    app_icon = QIcon()
    app_icon.addFile(os.path.join(current_dir, "icons", "image.svg"), QtCore.QSize(1024, 1024))
