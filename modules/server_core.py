...
class Client(messaging.ConnectionManager):
    ...
    def connect(self, client_selector, client_socket, client_addr):
        ...
        # Устанавливаем connected ДО super().connect(),
        # чтобы _resend_requests() не сыпал warning'ами
        self.connected = True

        super().connect(client_selector, client_socket, client_addr)

        # Запрашиваем id. on_connect вызовется из _got_id — ПОСЛЕ on_first_connect.
    ...
    def _got_id(self, _client, value):
        ...
        if old_id is None:
            # Сначала добавляем клиента в модель (появляется строка в таблице)
            if self.on_first_connect:
                self.on_first_connect(self)
            # Затем запрашиваем technic_dir
            self.get_response("technic_dir", self._got_technic_dir)

        # А теперь — уведомляем GUI о смене состояния. Строка уже есть.
        if self.on_connect:
            self.on_connect(self)

    def _got_technic_dir(self, _client, value):
        self.technic_dir = value
    ...
    def close(self, inner=False):
        if not inner:
            ...
            # Внутреннее закрытие перед реконнектом: сокет просто закрываем,
            # состояние GUI не трогаем
            super()._close()
